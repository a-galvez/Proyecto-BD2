...
def etl_metodoEnvio():
    try:
        ...
        consulta = input("Ingrese la consulta SQL para extraer los métodos de envío:\n")

        df = pd.read_sql(consulta, engine_oltp)

        df = transformar_mayusculas(df, 'NombreMetodo')
  
        df_existentes = pd.read_sql("SELECT IdMetodoEnvio FROM DimMetodoEnvio", engine_olap)
