...
def etl_hechos():
    try:
        ...
        df_existentes = pd.read_sql("SELECT IdVenta FROM HechosVentas", engine_olap)
        nuevos = df[~df["IdVenta"].isin(df_existentes["IdVenta"])]

        # Carga
        nuevos.to_sql("HechosVentas", engine_olap, if_exists="append", index=False)
        print("ETL completado exitosamente para HechosVentas")
